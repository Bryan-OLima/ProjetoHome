...
Perguntas como “como está a temperatura da CPU?” ou “quanta memória disponível há?” acionam `system.get_metrics` mesmo se a classificação do modelo falhar. A resposta mostra os valores formatados e confiáveis; se uma fonte do Android não estiver disponível, ela é indicada como indisponível em vez de ser estimada. Os dados estruturados continuam disponíveis em **Detalhes técnicos**, fechados por padrão, para conferência. Cálculos também mostram o resultado estruturado, permitindo conferir a expressão que foi avaliada.
