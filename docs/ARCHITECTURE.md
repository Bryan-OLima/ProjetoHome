...
O primeiro fluxo público é `POST /api/assistant/query`. Ele recebe uma pergunta limitada e gera um `correlationId`. Consultas explícitas de métricas e expressões aritméticas reconhecíveis são encaminhadas diretamente às respectivas ferramentas; confirmação de disponibilidade da API e indisponibilidade conhecida de média de duração de requisições possuem respostas locais explícitas. Nenhum desses caminhos depende de uma decisão JSON do modelo. Os resultados de ferramentas são transformados em texto determinístico pelo backend: métricas usam unidades formatadas e cálculos mostram a expressão e o resultado. Outras perguntas seguem diretamente ao modelo para uma resposta natural com contexto factual curto. A resposta textual não recebe acesso a integrações, logs, banco ou arquivos e não pode afirmar dados atuais sem resultado autorizado. Não há endpoint público que aceite nome de ferramenta ou argumentos do navegador. Logs de `assistant.query` e `assistant.tool` guardam metadados correlacionados, duração e resultado, mas nunca a pergunta, prompt, argumentos ou saída do modelo.
