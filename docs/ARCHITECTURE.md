...
As primeiras ferramentas são `system.get_metrics` e `math.evaluate`. A primeira exige `monitoring.read`, não recebe argumentos e chama exclusivamente o coletor de métricas do backend, com prazo de 1,5 segundo. A segunda exige `math.evaluate`, recebe uma expressão de até 128 caracteres e usa um analisador aritmético local que aceita apenas números, parênteses e `+`, `-`, `*`, `/` e `%`, com prazo de 100 ms. Ela não usa `eval`, shell, rede ou filesystem. O modelo não recebe referência ao coletor, ao shell, ao SQLite ou ao filesystem. A orquestração do assistente é a única consumidora desse registro; não há endpoint público genérico para executar ferramentas.
...
O primeiro fluxo público é `POST /api/assistant/query`. Ele recebe uma pergunta limitada e gera um `correlationId`. Consultas explícitas de métricas e expressões aritméticas reconhecíveis são encaminhadas diretamente às respectivas ferramentas; confirmação de disponibilidade da API e indisponibilidade conhecida de média de duração de requisições possuem respostas locais explícitas. Nenhum desses caminhos depende de uma decisão JSON do modelo. Os resultados de ferramentas são transformados em texto determinístico pelo backend: métricas usam unidades formatadas e cálculos mostram a expressão e o resultado. O resultado estruturado da ferramenta não integra a resposta pública do assistente; somente a mensagem e os identificadores de rastreio atravessam essa fronteira. Outras perguntas seguem diretamente ao modelo para uma resposta natural com contexto factual curto. A resposta textual não recebe acesso a integrações, logs, banco ou arquivos e não pode afirmar dados atuais sem resultado autorizado. Não há endpoint público que aceite nome de ferramenta ou argumentos do navegador. Logs de `assistant.query` e `assistant.tool` guardam metadados correlacionados, duração e resultado, mas nunca a pergunta, prompt, argumentos ou saída do modelo.
