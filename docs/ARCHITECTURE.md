...
O primeiro fluxo público é `POST /api/assistant/query`. Ele recebe uma pergunta limitada, gera um `correlationId` e pede ao modelo uma decisão JSON estrita entre `system.get_metrics` e texto. O backend rejeita saída fora desse contrato, executa a ferramenta registrada com somente os identificadores de rastreio e aplica uma queda segura para consultas explícitas de métricas. Uma segunda geração formula a resposta natural com contexto factual curto e, somente quando houver ferramenta, com o resultado sanitizado dela. A resposta textual não recebe acesso a integrações, logs, banco ou arquivos e não pode afirmar dados atuais sem resultado autorizado. Não há endpoint público que aceite nome de ferramenta ou argumentos do navegador. Logs de `assistant.query` e `assistant.tool` guardam metadados correlacionados, duração e resultado, mas nunca a pergunta, prompt, argumentos ou saída do modelo.
