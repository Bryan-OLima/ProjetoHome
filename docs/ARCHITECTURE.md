...
O primeiro fluxo público é `POST /api/assistant/query`. Ele recebe uma pergunta limitada, gera um `correlationId` e pede ao modelo uma decisão JSON estrita entre `system.get_metrics`, uma resposta textual limitada sobre o próprio Projeto Home e `unsupported`. O backend rejeita saída fora desse contrato, executa a ferramenta registrada com somente os identificadores de rastreio e devolve dados estruturados apenas quando há ferramenta. A resposta textual não recebe acesso a integrações nem pode afirmar dados atuais. Não há endpoint público que aceite nome de ferramenta ou argumentos do navegador. Logs de `assistant.query` e `assistant.tool` guardam metadados correlacionados, duração e resultado, mas nunca a pergunta, prompt, argumentos ou saída do modelo.
