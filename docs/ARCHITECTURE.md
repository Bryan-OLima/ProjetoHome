...
O primeiro fluxo público é `POST /api/assistant/query`. Ele recebe uma pergunta limitada, gera um `correlationId` e pede ao modelo uma decisão JSON estrita entre `system.get_metrics`, `math.evaluate` e texto. O backend rejeita saída fora desse contrato. Consultas explícitas de métricas e expressões aritméticas reconhecíveis são encaminhadas diretamente às respectivas ferramentas, sem depender da primeira decisão do modelo. Os resultados de ferramentas são transformados em texto determinístico pelo backend: métricas usam unidades formatadas e cálculos mostram a expressão e o resultado. O modelo é usado somente para respostas gerais com contexto factual curto. A resposta textual não recebe acesso a integrações, logs, banco ou arquivos e não pode afirmar dados atuais sem resultado autorizado. Não há endpoint público que aceite nome de ferramenta ou argumentos do navegador. Logs de `assistant.query` e `assistant.tool` guardam metadados correlacionados, duração e resultado, mas nunca a pergunta, prompt, argumentos ou saída do modelo.
